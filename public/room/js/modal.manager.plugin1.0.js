...
(function($,w){
	...
	'<p><i class="icon-info-sign" style="font-size:2em;color:#89b577"></i> @{msgContent}</p></div>  <div class="modal-footer"><a role="button" class="btn btn-warning btn-sm JConfirmBtn">确定</a></div>';

	var confirmTemplate='<div class="modal-header"><h3 class="modal-title">@{title}</h3></div><div class="modal-body">'+
	'<p><i class="icon-info-sign" style="font-size:2em;color:#89b577"></i> @{msgContent}</p></div>  <div class="modal-footer"><a role="button" class="btn btn-default btn-sm JCancelBtn">取消</a>'+
	'<a role="button" class="btn btn-warning btn-sm JConfirmBtn">确定</a></div>';
	$.extend(dialog,{
...
//			bootbox.alert(msg);
		...
		fConfirm:function(title,content,callback){
			...
			var config={content:confirmContent,width:350};
			var modal=this.fCreate(config);
			modal.$element.find(".JConfirmBtn").click(function(){
				modal.hide();
				if(callback){
