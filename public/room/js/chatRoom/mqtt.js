...
function mqttManager() {
    var hostname = mqttOpt.host;
    var port = useSSL ? 8084 : 8083;
    var clientId = uuid;
    var path = '';
    var user = '';
    var pass = '';
    ...
    var lastWillTopic = 'pyjy/allroom/chat';
    var lastWillQos = qos;
    ...
    var lastWillMessage = JSON.stringify(payload2);
...
function onConnect() {
    ...
    for (var i = 0; i < topic.length; i++) {
        client.subscribe(topic[i], {qos: Number(qos), onSuccess: subscribeSuccess});
        console.info('订阅主题: ', topic[i], '. QoS: ', qos);
    }
}
...
function onMessageArrived(message) {
    try {
        console.log(message.topic + " 消息：" + message.payloadString);
        var tp = message.topic;
        var tps = tp.split('/');
        if (tp.indexOf('/chat') > -1 && (tps[1] == room_id)) {//聊天室消息
            onChat(message.payloadString);
        } else if (tp.indexOf('allroom') > -1) {
            onChat(message.payloadString);
        }
    } catch (e) {
        console.log(e);
...
$(document).on('click',".lookList .follow",function (e) {
    ...
    if(onlineUserId == userInfo.id){
        layer.msg(errMsg[9], {shift: 6}); return;
    }
    var userId = onlineUserId;
...
function sayLike() {
    ...
    client.publish('pyjy/' + room_id + '/chat', JSON.stringify(payload2), 1);
...
$('.more-message').on('click', function () {
    $.ajax({
        ...
        url: '/generic/chatLog',
        dataType: 'json',
        data: {room_id: room_id, p: page, msg_id: lastMsgId},
        success: function (msg) {
            if (msg.code == 1) {
                var html = '';
                if(msg.list != ''){
...
function hiddenMsgList(){
    ...
    var maxLen = 9;
    console.log(page,getTime() - pageTime);
