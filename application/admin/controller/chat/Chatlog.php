...
class Chatlog extends Backend
{
    ...
    public function index()
    {
        ...
        $s_user = model('user')->get($ids) ;
        $this->assign('username', $s_user ? $s_user['username'] : '');
...
//            }
     ...
     * ip黑名单
     */
    public function panip()
    {
        $userids = $this->request->param('user_id');
        $type = $this->request->param('type');

        if ($this->request->isAjax()) {
            $loginip = model('user')->where('id', 'in', $userids)->column('loginip');
            if($loginip){
                if($type ==  2){
                    model('ShieldIp')->where(['ip' => $loginip[0]])->delete();
                }else{
                    model('ShieldIp')->insert(['ip' => $loginip[0], 'createtime' => time()]);
                    $opt2 = [
                        'topic' => 'pyjy/allroom/chat',
                        'payload' => [
                            'action'    => 'panIp',
        ...
        $this->error(__('Parameter %s can not be empty', $this->request->isAjax()));
    }
}
